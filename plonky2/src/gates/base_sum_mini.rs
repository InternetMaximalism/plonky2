...
pub struct BaseSumMiniGate<const B: usize> {
    pub num_limbs: usize,
}

impl<const B: usize> BaseSumMiniGate<B> {
...
impl<F: RichField + Extendable<D>, const D: usize, const B: usize> Gate<F, D> for BaseSumMiniGate<B> {
    fn id(&self) -> String {
        format!("{self:?} + Base: {B}")
    }
...
impl<F: RichField, const B: usize> SimpleGenerator<F> for BaseSplitGenerator<B> {
    ...
    fn dependencies(&self) -> Vec<Target> {
        vec![Target::wire(self.row, BaseSumMiniGate::<B>::WIRE_SUM)]
    }

    fn run_once(&self, witness: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        let sum_value = witness
            .get_target(Target::wire(self.row, BaseSumMiniGate::<B>::WIRE_SUM))
        ...
        let limbs = (BaseSumMiniGate::<B>::START_LIMBS..BaseSumMiniGate::<B>::START_LIMBS + self.num_limbs)
